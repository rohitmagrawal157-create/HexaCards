...
function CardsPanel({
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-3">
        {[
          ...
          },
          {
            ...
            hint: orders.length ? "Ready to track" : "None yet",
            icon: Package,
          },
          {
            label: "Total spent",
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#FAFAF8] text-[#BC7C10]">
                  <Icon className="h-4 w-4" strokeWidth={1.75} />
                </span>
              </div>
              <p className="mt-3 font-dashboard text-[1.65rem] font-extrabold tracking-[-0.03em] tabular-nums text-[#141414]">
                {stat.value}
              </p>
              <p className="mt-1 text-xs text-[#8a8174]">{stat.hint}</p>
            </div>
          );
        })}
      </div>
      ...
      {latestOrder ? (
        <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-black/[0.06] bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,0.03)] sm:p-5">
          <div className="flex items-start gap-3">
            <span className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[#FFF8ED] text-[#BC7C10]">
              <Truck className="h-5 w-5" strokeWidth={1.75} />
            </span>
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <span
                  className={`rounded-md px-2 py-0.5 text-[10px] font-bold tracking-wide uppercase ${statusTone(latestOrder.status)}`}
                >
                  {statusLabel(latestOrder.status)}
                </span>
                <span className="text-xs text-[#8a8174]">{latestOrder.id}</span>
              </div>
              <p className="mt-1.5 text-sm font-semibold text-[#141414]">
                {latestOrder.productTitle}
              </p>
              <p className="mt-0.5 text-xs text-[#6b6560]">
                Placed {formatOrderDate(latestOrder.createdAt)} · Qty{" "}
                {latestOrder.qty}
              </p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={onTrack}
              className="rounded-lg border border-black/[0.08] bg-white px-4 py-2 text-[13px] font-semibold text-[#141414] transition-colors hover:bg-[#FAFAF8]"
            >
              Track order
            </button>
            <span className="rounded-lg bg-[#141414] px-4 py-2 font-dashboard text-[13px] font-bold tabular-nums text-white">
              ₹{latestOrder.total.toLocaleString("en-IN")}
            </span>
          </div>
        </div>
      ) : (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-dashed border-[#BC7C10]/35 bg-[#FFFCF7] px-5 py-4">
          <div>
            <p className="text-sm font-semibold text-[#141414]">
              No orders in progress
            </p>
            <p className="mt-0.5 text-xs text-[#6b6560]">
              Place an order to start tracking shipment status here.
            </p>
          </div>
          <Link
            href="/products"
            className="inline-flex items-center gap-1.5 rounded-lg bg-[#BC7C10] px-4 py-2 text-[13px] font-semibold text-white hover:bg-[#9a650d]"
          >
            Shop products
            <ArrowUpRight className="h-3.5 w-3.5" />
          </Link>
        ...
        <article className="overflow-hidden rounded-xl border border-black/[0.06] bg-white shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
          <div className="relative aspect-[16/10] bg-[#111]">
            ...
            <div className="absolute right-4 bottom-4 left-4 text-white">
              ...
              <p className="mt-0.5 text-sm text-white/80">Hexa NFC Business Card</p>
              <p className="mt-1 truncate font-mono text-[11px] text-white/60">
                hexacards.com/u/{user.phone.slice(-4)}
              </p>
            </div>
          </div>
  ...
  if (orders.length === 0) {
    return (
      <EmptyPanel
        icon={ShoppingBag}
        title="No orders yet"
        text="After you place an order at checkout, tracking details will appear here."
        action={{ href: "/products", label: "Browse products" }}
      />
    );
  }
